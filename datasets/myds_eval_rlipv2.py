...
class GroupHOIMetrics:
    ...
    def build_gt_groups(self, gt_img: Dict[str, Any]) -> List[GroupDict]:
        ...
        if not raw_groups:
            ...
            if syn:
                return syn
        # ¼æÈÝ string/int id »ìÓÃ£¨ÀýÈç entity_id=1£¬µ«³ÉÔ±ÀïÐ´³É"1"£©
        ...
        def _iter_member_like_items(group_item: Dict[str, Any]):
            ...
            # ¼æÈÝ¡°±âÆ½ group¡±£ºgroup ±¾Éí¾ÍÊÇÒ»¸ö³ÉÔ±
        ...
        # Èç¹ûÊý¾Ý¼¯ÖÐÃ»ÓÐÏÔÊ½ group ±ê×¢£¬Ôò´Ó HOI ±ê×¢×Ô¶¯ÍÆ¶Ï group
        # ¹æÔò£º°Ñ (subject_id, object_id, action) ¿´×÷Í¼ÉÏµÄ±ß£¬°´Á¬Í¨·ÖÁ¿¾ÛºÏÎª group¡£
        if not groups:
    ...
    def _build_pred_groups_heuristic_graph(self, pred_hois: List[Dict[str, Any]], pred_boxes: List[Dict[str, Any]]) -> List[GroupDict]:
        ...
        # Îª±ÜÃâ¡°Á¬Í¨·ÖÁ¿¹ý´óµ¼ÖÂ strict match ¼«µÍ¡±£¬²¹³äµ¥±ß group ºòÑ¡¡£
        # Õâ¶Ô GT ÖÐ´óÁ¿ size=1/2 µÄ group ÓÐÃ÷ÏÔ°ïÖú£¬Ò²ÄÜÈÃ Recall@K ÕæÕýËæ K Ôö³¤¡£
        singleton_groups = []
        singleton_best = {}
    ...
    def evaluate(self, preds: List[Dict[str, Any]], gts: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        out = {
            ...
            # ¼æÈÝÈÕÖ¾¼ìË÷Ï°¹ß£ºÍ¬Ê±±©Â¶ mAP / mean max recall ÃüÃû
        ...
        if _is_main_process() and total_gt_groups == 0 and total_raw_group_items > 0:
            ...
            # ´òÓ¡Ò»ÌõÑùÀý°ïÖú¿ìËÙ¶¨Î» schema ÎÊÌâ
            sample = None
            for gt_img in gts:
                rg = (
                    gt_img.get("group_interactions")
...
class MyDatasetEvaluator:
    ...
    def _build_from_preds_gts(self, preds, gts) -> None:
        ...
        for idx, img_preds in enumerate(preds):
            ...
            total_pairs = int(num_queries * num_verbs)
            max_keep = max(self.max_hois if self.max_hois > 0 else total_pairs,
                           self.group_max_hois if self.group_max_hois > 0 else total_pairs)
            max_keep = min(max_keep, total_pairs)

            scores_flat = verb_scores.reshape(-1)
            if max_keep < total_pairs:
                topk_idx = np.argpartition(-scores_flat, max_keep - 1)[:max_keep]
                topk_idx = topk_idx[np.argsort(-scores_flat[topk_idx])]
            else:
                topk_idx = np.argsort(-scores_flat)

            pred_hois = []
            for flat_idx in topk_idx:
                q_idx = int(flat_idx // num_verbs)
                v_idx = int(flat_idx % num_verbs)
                pred_hois.append({
                    "subject_id": int(sub_ids[q_idx]),
                    "object_id": int(obj_ids[q_idx]),
                    "action": v_idx,
                    "score": float(scores_flat[flat_idx]),
                })
    ...
    def _build_rare_nonrare_sets(self, eval_gts_raw) -> Tuple[Set[Triplet], Set[Triplet]]:
        ...
        if _is_main_process():
            ...
            print(f"[MyDatasetEvaluator] eval_triplets={len(eval_space)}, triplets_with_train_count¡Ö{shared}")
